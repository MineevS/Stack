...
//LinkedList::LinkedList (this=0x7fffffffe3d0) at LinkedList.cpp:29
//(gdb) p *this
//$4 = {_head = 0x7fffffffe4c0, _size = 0}
//(gdb) p *this->_head
//$5 = {value = 4.9406564584124654e-324, next = 0x7fffffffe716}
...
{
	if (_size == 0) //Если размер списка равен нулю,тогда вставить элемент в начало.
	{
		pushFront(value);
		return;
	}
	insert(_size, value);//Иначе вставить элемент по индексу.
}
//+
void LinkedList::pushFront(const ValueType& value)
{
	_head = new Node(value, _head);//?
	++_size;//Увеличеть размер листа на 1.
}
